...
def process_file(files):
    ...
    for f in files:
        try:
            df = pd.read_excel(os.path.join(path_to_watch, f), sheet_name=0, skiprows=8)
            if 'Neraca' in f:
                print("Memproses file neraca")
                ...
                try:
                    ...
                    expectedcols = ['keterangan', 'key', 'amount', 'amount_compare']
                    for fc in filecols:
                ...
                try:
                    query = f"""DELETE FROM neraca"""
                    engine.execute(
                        sa_text(query).execution_options(autocommit=True)
                    )
                    df.fillna(0)
                    df.to_sql('neraca', engine,
                              if_exists='append', index=False)
                    sendMessage('successfully save valid data of neraca')
                except Exception as e:  
                    print(e)
            if 'Laba_Rugi' in f:
                print("Memproses file laba rugi")
                ...
                try:
                    query = f"""DELETE FROM laba_rugi"""
                    engine.execute(
                        sa_text(query).execution_options(autocommit=True)
                    )
                    df.fillna(0)
                    df.to_sql('laba_rugi', engine,
                              if_exists='append', index=False)
                    sendMessage('successfully save valid data of laba rugi')
                except Exception as e:
                    print(e)
